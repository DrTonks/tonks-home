...
try {
  ...
  const toolbar=modal.locator('.feedback-composer-toolbar')
  const emojiToggle=toolbar.getByRole('button',{name:'选择表情',exact:true})
  const articleToggle=toolbar.getByRole('button',{name:'引用文章或小标题',exact:true})
  await page.evaluate(()=>document.documentElement.classList.add('dark'))
  await emojiToggle.click()
  const darkBackground=await page.locator('.community-emoji-picker').evaluate(el=>getComputedStyle(el).backgroundColor)
  assert(/^rgb\(/.test(darkBackground),'dark emoji panel must be opaque')
  assert(darkBackground.match(/\d+/g).slice(0,3).every(n=>Number(n)<80),'dark emoji panel must have a dark background')
  await articleToggle.click()
  assert.equal(await page.locator('.community-emoji-picker').count(),0,'article opening must close emoji')
  assert.equal(await page.locator('.community-article-picker:visible').count(),1)
  await emojiToggle.click()
  assert.equal(await page.locator('.community-article-picker:visible').count(),0,'emoji opening must close article')
  await articleToggle.focus()
  await page.keyboard.press('Enter')
  assert.equal(await page.locator('.community-emoji-picker').count(),0,'keyboard article opening must close emoji')
  await page.keyboard.press('Escape')
  ...
  await card.waitFor()
  const assertCardDocked=async()=>{
    const layout=await card.evaluate(el=>{
      const panel=el.getBoundingClientRect()
      const footer=el.closest('.feedback-composer').getBoundingClientRect()
      return {gap:footer.top+el.closest('.feedback-composer').clientTop-panel.bottom,left:panel.left-footer.left,background:getComputedStyle(el).backgroundColor}
    })
    assert(Math.abs(layout.gap-8)<1,'feedback card must dock above input on every opening: '+JSON.stringify(layout))
    assert(Math.abs(layout.left-12)<1,'feedback card must stay inside input width')
    assert(/^rgb\(/.test(layout.background),'feedback card background must be opaque')
  }
  await assertCardDocked()
