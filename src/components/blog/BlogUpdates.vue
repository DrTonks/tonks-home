...
function handleCardClick(event: MouseEvent) {
  ...
  if (target.closest('a, button, input, textarea, select, [role="tab"], [data-no-card-open]')) {
    return
  }
  openCommunity()
}
...
<template>
  ...
  >
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="flex items-center gap-1">
        <button
          type="button"
          class="card-action-hint community-action-hint"
          aria-haspopup="dialog"
          :aria-expanded="communityOpen"
          @click.stop="openCommunity"
        >
  ...
  <BlogCommunityDialog v-model:open="communityOpen" />
</template>
...
.community-action-hint:focus-visible {
