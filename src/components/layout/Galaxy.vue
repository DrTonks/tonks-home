...
/**
 * Galaxy 银河星空背景（WebGL / ogl）。改编自 galaxy.md：
 * - 鼠标监听改到 window（作为全屏底层背景时，canvas pointer-events:none 仍能响应）
 * - 动态参数（hueShift/glowIntensity/starSpeed/saturation/twinkle/density）每帧读 props
 *   并 lerp 平滑更新对应 uniform：实时响应、不重建 WebGL（可接音频联动），
 *   替代原组件对 props 的 deep watch 重建（那会销毁重建整个 WebGL、开销大）
 * - 尊重 prefers-reduced-motion：静止星空、不监听鼠标
 * - 默认配色按项目品牌调（偏蓝 hueShift + 低饱和微品牌色）
 */
...
const disableAnim = props.disableAnimation || reduceMotion
const enableMouse = props.mouseInteraction && !reduceMotion
...
const setup = () => {
  ...
  const renderer = new Renderer({ alpha: props.transparent, premultipliedAlpha: false })
  ...
  let curStarSpeed = props.starSpeed
  ...
  function update(t: number) {
    ...
    // 每帧读取最新 props 并平滑逼近（父组件改参数即实时生效，可接音频联动）
    const k = 0.05
    curStarSpeed += (props.starSpeed - curStarSpeed) * k
    ...
    if (!disableAnim) {
      ...
      starAccum += dt * curStarSpeed * 0.1
      program.uniforms.uStarSpeed.value = starAccum
    }
...
<template>
  <div ref="ctnDom" class="w-full h-full" v-bind="$attrs" />
</template>
